...
class LottoTest {
    ...
    void 천원단위로_로또_발급하는지_체크(final int money) throws Exception {
        ...
        //then
        Assertions.assertThat(results).hasSize(money / lotto.getLottoPrice());
    }
    ...
    void 로또_번호는_6자리_체크() throws Exception {
        ...
        //then
        boolean hasInvalidsize = results.stream()
            .anyMatch(list -> list.size() != lotto.getLottoSize());
        
        Assertions.assertThat(hasInvalidsize).isFalse();
    }
    ...
    void 로또_숫자범위_1부터_45까지_체크(final int money) throws Exception {
        ...
        boolean hasInvalidValue = results.stream().flatMap(List::stream)
            .anyMatch(value -> value < 0 || value
                > Lotto.BOUND.MAX_BOUND_OF_LOTTO.getBoundary()); //TODO - stream: List<List<Integer>> 스트림 어케 만들지?
        
        Assertions.assertThat(hasInvalidValue).isFalse();
    }
    ...
    void 모든_로또번호가_유일한지_체크(final int money) throws Exception {
        ...
        for (List<Integer> lotto : results) {
            ...
            Assertions.assertThat(uniqueNumbers).hasSameSizeAs(lotto);
        }
    }
    ...
    void 긁은복권모음이_null이면_throw_Exception() {
        ...
        //when & then
        Assertions.assertThatThrownBy(() -> {
                lotto.verifyLottoResult(당첨번호, null);
