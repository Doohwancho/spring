...
class ThreadPoolsTest {
    ...
    void testNewFixedThreadPool() {
        ...
        assertThat(executor.getPoolSize()).isEqualTo(expectedPoolSize); //Executors.newFixedThreadPool(2); 했으니까 쓰레드 풀에 쓰레드 3개 넣어도 병렬로 작동하는 쓰레드 최대 갯수는 2개
        assertThat(executor.getQueue().size()).isEqualTo(expectedQueueSize); //쓰레드 풀에서 최대로 2개 도니까, 나머지 1개는 task Queue에서 대기 중이니까 1개
    }
    ...
    void testNewCachedThreadPool() {
        ...
        assertThat(executor.getPoolSize()).isEqualTo(expectedPoolSize); //newCachedThreadPool는 동시에 병렬로 도는 쓰레드 제한이 없어서 3개 넣으면 3개 돌고, 10개 넣으면 10개 동시에 돔
        assertThat(executor.getQueue().size()).isEqualTo(expectedQueueSize); // 쓰레드풀에 넣은만큼 다 병렬로 도니까, task queue에서 기다리는 쓰레드가 없음. 그래서 0개
    }
