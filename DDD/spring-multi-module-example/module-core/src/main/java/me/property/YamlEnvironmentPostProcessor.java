...
public class YamlEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    private static final String[] propertyUris = { "classpath*:*.yml" }; //like application.yml
    private static final String[] acceptsProfiles = { "h2", "mysql" }; //define profiles
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        if (isNotValidProfileActive) {
            environment.setActiveProfiles("h2"); //if not included in default profile(h2, mysql), set default profile to h2
        }
