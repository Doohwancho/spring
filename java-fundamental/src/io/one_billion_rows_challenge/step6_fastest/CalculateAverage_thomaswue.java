...
class CalculateAverage_thomaswue {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        try (var fileChannel = FileChannel.open(java.nio.file.Path.of(FILE), java.nio.file.StandardOpenOption.READ)) {
            ...
            final long fileStart = fileChannel.map(FileChannel.MapMode.READ_ONLY, 0, fileSize, java.lang.foreign.Arena.global()).address(); //Arena is jvm에서 JNI, low level native code더 잘 조작하게 하는 프로젝트에서 나온 라이브러리. memory allocation, de-allocation 담당함
    ...
    //ensure that memory cleanup can occur while the main process returns the result
    ...
    private static void parseLoop(AtomicLong counter, long fileEnd, long fileStart, List<Result> collectedResults) {
        Result[] results = new Result[HASH_TABLE_SIZE]; //131,072, 넉넉한 hash table size + avoid hash-collision 만큼 충분히 큼 (city 이름이 1000개니까, 13만개면 hash-collision 최대한 피할 수 있지 않을까? -> memory 엄청 써서 cpu optimize 한 방법
        while (true) {
    ...
    private static long nextNewLine(long prev) {
        while (true) {
            long currentWord = Scanner.UNSAFE.getLong(prev); //read 8 byte, 64bits, direct memory access using UnSafe
