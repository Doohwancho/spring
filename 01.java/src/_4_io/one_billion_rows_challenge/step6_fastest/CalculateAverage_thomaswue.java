//package io.one_billion_rows_challenge.step6_fastest;
///*
...
//
///**
// * The solution starts a child worker process for the actual work such that clean up of the _3_syntax_details.memory mapping can occur
// * while the main process already returns with the result. The worker then _3_syntax_details.memory maps the input file, creates a worker
...
// *  Alfonso² Peterssen for suggesting _3_syntax_details.memory mapping with performance.unsafe and the subprocess idea
...
// */
//class CalculateAverage_thomaswue {
...
//    private static final int SEGMENT_SIZE = 1 << 21; //2^21 = 2,097,152 = 2MB만큼 1.4GiB 파일을 청크로 쪼개어 병렬 처리
//    private static final int HASH_TABLE_SIZE = 1 << 17; //2^17 = 131,072, = max_city가 1000개인데, hash table 크기를 13만으로 정한건, to avoid hash collision -> latency
//
...
//            final long fileStart = fileChannel.map(FileChannel.MapMode.READ_ONLY, 0, fileSize, java.lang.foreign.Arena.global()).address(); //Arena is jvm에서 JNI, low level native code더 잘 조작하게 하는 프로젝트에서 나온 라이브러리. _3_syntax_details.memory allocation, de-allocation 담당함
//            final long fileEnd = fileStart + fileSize;
//            final AtomicLong cursor = new AtomicLong(fileStart); //thread-safe in multi-thread concurrent environment, faster than synchronized locking
//
...
//    }
//
//    //spawns a worker subprocess to handle the actual data processing
//    //ensure that _3_syntax_details.memory cleanup can occur while the main process returns the result
...
//    }
//
//    //all worker threads with different segments 에서 result를 모아 TreeMap 에 sequential, sorted
...
//    }
//
//    //parsing & data handling methods
//    //parse file into multiple segments so that they can be handled by multiple threads
//    //it divides each segments into 3 parts, and uses cursor to process these parts simultaneously
//    private static void parseLoop(AtomicLong counter, long fileEnd, long fileStart, List<Result> collectedResults) {
//        Result[] results = new Result[HASH_TABLE_SIZE]; //131,072, 넉넉한 hash table size + avoid hash-collision 만큼 충분히 큼 (city 이름이 1000개니까, 13만개면 hash-collision 최대한 피할 수 있지 않을까? -> _3_syntax_details.memory 엄청 써서 cpu optimize 한 방법
//        while (true) {
//            long current = counter.addAndGet(SEGMENT_SIZE) - SEGMENT_SIZE; //2^21 = 2,097,152 -> 2MB 만큼 파일을 chunk로 쪼갬
//            if (current >= fileEnd) {
//                return;
//            }
//
...
//            }
//
//            //2MB chunk from 1.4GiB file 을 1/3 으로 다시 쪼갬. 그럼 약 700KB 3덩이로 쪼개는거네
...
//                long word1b = scanner1.getLongAt(scanner1.pos() + 8); //long = 8byte, add another 8byte to get next line
...
//
//            //loop unrolling
//            //optimization technique -> reduces overhead of loop control (checking loop condition and incrementing counters)
//            //can increase CPU's branch prediction accuracy, by explicitly writing out the operations for each scanner rather than using a loop
//            while (scanner1.hasNext()) {
//                long word = scanner1.getLong(); //8byte를 읽어서
//                long pos = findDelimiter(word); //';'(delimiter)의 위치 파악
//                long wordB = scanner1.getLongAt(scanner1.pos() + 8); //read next 8 byte
//                long posB = findDelimiter(wordB); //';'의 위치 파악
...
//
//    //parsing & data handling methods
//    //creates Result Object for a given city from input
//    //it handles hash collisions and efficiently searches for existing resuts
...
//
//    //parsing & data handling methods
//    //find the position of next newline character('\n', 0x0A in ASCII) in the mapped file,
//    // aiding in determining segment boundaries for parsing
//    private static long nextNewLine(long prev) {
//        while (true) {
//            long currentWord = Scanner.UNSAFE.getLong(prev); //read 8 byte, 64bits, direct _3_syntax_details.memory access using UnSafe
//            long input = currentWord ^ 0x0A0A0A0A0A0A0A0AL;
//            long pos = (input - 0x0101010101010101L) & ~input & 0x8080808080808080L; //find exact position of newline char
//            if (pos != 0) {
//                prev += Long.numberOfTrailingZeros(pos) >>> 3; //convert bits -> byte by diving by 8 //convert bits -> byte by diving by 8 //convert bits -> byte by diving by 8 //convert bits -> byte by diving by 8
//                break;
//            }
//            else {
//                prev += 8;
...
//
//    //parsing & data handling methods
//    //parse number from input, using branchless programming to efficiently convert ASCII to numberic value
...
//
//    //parsing & data handling methods
//    //minimum, maximum, avg temperature handling
...
//
//    //helper function
//    //converts hashcode into index for result array
...
//
//    //helper function
//    // identifies position of delimiter(';' in this case) using bitmask to scan multiple characters simultaneously
...
//
//    //helper function
//    //creates new Result object for a city name that hasn't been encountered yet
