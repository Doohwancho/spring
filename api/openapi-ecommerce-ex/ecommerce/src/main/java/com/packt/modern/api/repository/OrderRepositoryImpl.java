...
import com.packt.modern.api.exception.custom.ResourceNotFoundException;
...
/*
  TODO - Q. 왜 implements OrderRepositoryExt함? implements OrderRepository 안하고?

  A. implements OrderRepository 하면, save(), findById(), delete() 등
  CrudRepository<OrderEntity, UUID> 인터페이스에 정의된 메서드 다 implements 받아야 함.

  custom method인 insert() 하나만 만들고 싶으니까, OrderRepositoryExt에 insert() 하나 만들고, implements 받아서 만드는 것
 */
...
public class OrderRepositoryImpl implements OrderRepositoryExt {
  ...
  public Optional<OrderEntity> insert(NewOrder m) {
    ...
    OrderEntity entity = (OrderEntity) em.createNativeQuery("SELECT o.* FROM ecomm.orders o WHERE o.customer_id = ? AND o.order_date >= ?", OrderEntity.class)
